...
public:
    ...
    void init_opencl()
    {
        ...
        if (platforms.empty())
        {
            spdlog::error("JacobiGpuTexture: No OpenCL platforms found");
            return;
        }
        ...
        if (devices.empty())
        {
            spdlog::warn("JacobiGpuTexture: No GPU devices, trying CPU");
            platforms[0].getDevices(CL_DEVICE_TYPE_CPU, &devices);
        }

        if (devices.empty())
        {
            spdlog::error("JacobiGpuTexture: No devices found");
            return;
        }
        ...
        {
            spdlog::error("JacobiGpuTexture: Build error: {}", program.getBuildInfo<CL_PROGRAM_BUILD_LOG>(device));
            return;
        }

        kernel = cl::Kernel(program, "jacobi_texture");
        is_initialized = true;
        spdlog::info("JacobiGpuTexture: Initialized with {}", device.getInfo<CL_DEVICE_NAME>());
    }
    ...
    float *run()
    {
        if (!is_initialized)
        {
            spdlog::error("JacobiGpuTexture: Not initialized");
            return nullptr;
        }
        ...
        spdlog::info("JacobiGpuTexture: {} iterations in {} ms", iterations, ms);

        return data;
    }
};

JacobiGpuTexture::JacobiGpuTexture(uint32_t grid_size, uint32_t iterations)
    : JacobiRunner(grid_size, iterations), _impl(std::make_unique<Impl>(grid_size, iterations))
{
}

JacobiGpuTexture::~JacobiGpuTexture() = default;

float *JacobiGpuTexture::run()
{
    return _impl->run();
}

uint32_t JacobiGpuTexture::grid_size() const
{
    return _impl->grid_size;
}

uint32_t JacobiGpuTexture::iterations() const
{
